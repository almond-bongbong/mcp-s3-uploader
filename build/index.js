...
const AWS_REGION = process.env.AWS_REGION ?? process.env.AWS_DEFAULT_REGION ?? 'ap-northeast-2';
const DEFAULT_PREFIX = (process.env.S3_PREFIX ?? 'codex-v0/').replace(/^\/*/, '');
const DEFAULT_EXPIRES_IN = Number.parseInt(process.env.URL_EXPIRES_IN ?? '86400', 10);
...
function createObjectKey(prefix, ext) {
    const now = new Date();
...
/** S3 upload helpers */
async function uploadImageAndGetUrl(inputPath, opts) {
    ...
    const url = await getSignedUrl(s3, new GetObjectCommand({ Bucket: S3_BUCKET, Key: key }), { expiresIn: opts.expiresIn });
    return {
        url,
        bucket: S3_BUCKET,
        key,
...
async function execFileBuffer(cmd, args, maxBuffer = 30 * 1024 * 1024) {
    return new Promise((resolve, reject) => {
        execFile(cmd, args, { encoding: 'buffer', maxBuffer }, (error, stdout, stderr) => {
            if (error) {
...
async function saveClipboardImageToTempPng() {
    if (process.platform !== 'darwin') {
        throw new Error('upload_clipboard_image currently supports macOS only.');
    }
    const tmpPath = path.join(os.tmpdir(), `mcp-clipboard-${crypto.randomUUID()}.png`);
    // 1) Try pngpaste (best)
    try {
 ...
 */
server.registerTool('upload_clipboard_image', {
...
async function main() {
    ...
    console.error(`[mcp-s3-uploader] running on stdio (region=${AWS_REGION}, bucket=${S3_BUCKET}, prefix=${DEFAULT_PREFIX})`);
}
main().catch((e) => {
